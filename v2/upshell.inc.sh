...
upshell_generate_dot_profile() {
   ...
   src="$UPSHELL_CONFIG_HOME"/custom.profile
   if [ -e "$src" ]; then
      echo >> "$rc"
      cat "$src" >> "$rc"
   fi
...
upshell_generate_dot_bashrc() {
   ...
   src="$UPSHELL_CONFIG_HOME"/custom.bashrc
   if [ -e "$src" ]; then
      echo >> "$rc"
      cat "$src" >> "$rc"
   fi
...
upshell_generate_dot_bash_profile() {
   ...
   src="$UPSHELL_CONFIG_HOME"/custom.bash_profile
   if [ -e "$src" ]; then
      echo >> "$rc"
      cat "$src" >> "$rc"
   fi
...
upshell_generate_dot_bash_login() {
   rc="$UPSHELL_GENERATED_HOME"/.bash_login
   {
      echo '# STOP! Do not put anything in this file; with Upshell, Bash will not read'
      echo '# it. Bash would only read this file if ~/.bash_profile did not exist.'
   } > "$rc"

   src="$UPSHELL_CONFIG_HOME"/custom.bash_login
   if [ -e "$src" ]; then
      {
         echo
         echo "$src":
...
upshell_generate_dot_bash_logout() {
   rc="$UPSHELL_GENERATED_HOME"/.bash_logout
   {
      echo '# STOP! Do not put code here. With Upshell, it will not do what'
      echo '# you might think. See the Upshell documentation.'
   } > "$rc"

   src="$UPSHELL_CONFIG_HOME"/custom.bash_logout
   if [ -e "$src" ]; then
      {
         echo
         echo "$src":
